function x_triangle = GenerateTrianglePulses(t_axis,T_sq,E_bit,Fs,x_bits,type)
%
% Inputs:
%   t_axis:     Time axis
...
% Outputs:
%   x_triangle:   The sequence of samples corresponding to the pulse shaping
%               of the input bits using a triangle pulse shape
%
% This function takes an input a sequence of bits, x_bits. It then
% generates a sequence of samples in x_triangle, corresponding to the pulse
...
%       x_triangle is always equal in dimention to t_axis.
...
one_triangle = zeros(1,N_sq); % array of zeros, length = N_sq
...
switch type
    case ('bipolar')
        ...
        x_triangle = zeros(1,length(t_axis));
    ...
    case ('unipolar')
        ...
        for n = 1:length(x_bits)
           ...
           %up edge
           for s = 1:N_sq/2
                x_triangle( 1, i+s ) = s*step ;  
           end
        
            %down edge
            for s = 1:N_sq/2
                x_triangle( 1, i+s+N_sq/2 ) = A-s*step ;  
            end
           end
